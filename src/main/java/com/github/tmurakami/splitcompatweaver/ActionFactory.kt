...
internal class ActionFactory(private val candidates: Set<String>) {
    fun createClassAction(source: File, status: Status, target: File): Action =
        when (status) {
            ADDED, CHANGED -> ReplaceClassAction(source, target, candidates)
            REMOVED -> DeleteAction(target)
            else -> NopAction
        }
}
